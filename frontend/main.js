...
form.addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`${backendUrl}/run`, {
            ...
            body: JSON.stringify({ goal: goal, intensity: intensity }),
        });
                ...
                setTimeout(() => {
                    ...
                    if (index === data.agent_output.length - 1) {
                        setTimeout(() => {
                            ...
                            renderMissionReport(goal, data.subtasks, backendUrl);
...
// Report Logic
const renderMissionReport = (goal, subtasks, backendUrl) => {
        ...
        if (missions.length > 0) {
            ...
            missions.slice(0, 5).forEach(m => {
                ...
                item.addEventListener('mouseenter', () => {
                    ...
                    item.style.borderColor = 'hsla(var(--primary), 0.5)';
                });
                ...
                item.addEventListener('click', () => {
                    document.getElementById('goal').value = m.goal;
                    renderMissionReport(m.goal, m.subtasks, backendUrl);
                });
...
const setupUX = () => {
    ...
    style.innerHTML = `
        ...
        .history-item:hover i { transform: translateX(3px); color: hsla(var(--primary), 1) !important; }
    `;
